...
export default function Navbar({}: Props): ReactElement {
  ...
  return (
    <header className="fixed z-10 flex items-center w-full h-16 pt-4 mb-4 bg-white border-b border-gray-200 shadow-xs header">
      <div className="flex justify-between w-10/12 mx-auto">
        <Link href="/">
          <div className="w-3/4">
            <ProgressiveImage
              originalImageSrc="assets/dejibimbola-logo.png"
        ...
        <nav className="hidden md:flex md:items-center nav">
          <div className="text-right">
            <ul className="flex nav__links">
              ...
              <li className="mx-4 font-sans text-sm font-bold transition duration-300 ease-in-out hover:text-yellow-600">
                <Link target="_blank" href="https://blog.dejibimbola.com">
                  Blog
                </Link>
                {/* Add opens in new tab Icon */}
                <NewTabIcon ml={1} />
              </li>
              <li className="mx-4 font-sans text-sm font-bold transition duration-300 ease-in-out hover:text-yellow-600">
        ...
        {!navBarCollapse && (
          <div
            className="self-center justify-self-end nav__collapse md:hidden"
            onClick={() => toggleNavbar(true)}
            onTouchStart={() => toggleNavbar(true)}
          >
            <motion.svg
      ...
      {navBarCollapse && (
        <motion.nav
          animate={navBarCollapse ? "open" : "closed"}
          variants={variants}
          ref={ref}
          className="absolute z-0 w-4/12 px-10 py-8 mt-56 bg-white shadow nav md:hidden"
        >
          <div className="text-right">
            <ul className="nav__links">
              ...
              <li className="mx-4 mb-5 font-sans text-sm font-bold transition duration-300 ease-in-out hover:text-yellow-600">
                <a target="_blank" href="https://blog.dejibimbola.com.ng">
                  Blog
                </a>
              </li>
              <li className="mx-4 mb-5 font-sans text-sm font-bold transition duration-300 ease-in-out hover:text-yellow-600">
